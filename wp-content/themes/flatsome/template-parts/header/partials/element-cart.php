...
<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php _e('Cart','flatsome-admin'); ?>" class="header-cart-link <?php echo get_flatsome_icon_class($icon_style, 'small'); ?>">

<?php } else if($cart_style == 'off-canvas') { ?>

<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="header-cart-link off-canvas-toggle nav-top-link <?php echo get_flatsome_icon_class($icon_style, 'small'); ?>" data-open="#cart-popup" data-class="off-canvas-cart" title="<?php _e('Cart','flatsome-admin'); ?>" data-pos="right">

<?php } ?>
  
<?php  if($cart_total || $cart_title) { ?>
<span class="header-cart-title">
  <?php if($cart_title) { ?> <?php _e('Cart','flatsome-admin'); ?> <?php } ?>
...
if(get_theme_mod('custom_cart_icon')) { ?>
  <span class="image-icon header-cart-icon" data-icon-label="<?php echo $woocommerce->cart->cart_contents_count; ?>">
    <img class="cart-img-icon" alt="<?php _e('Cart','flatsome-admin'); ?>" src="<?php echo do_shortcode(get_theme_mod('custom_cart_icon')); ?>"/> 
  </span><!-- .cart-img-inner -->
<?php } 
else { ?>
  <?php if(!$icon_style) { ?>
...
<?php if($cart_style == 'off-canvas') { ?>
  ...
  <div class="cart-popup-inner inner-padding">
      <div class="cart-popup-title text-center">
          <h4 class="uppercase"><?php _e('Cart','flatsome-admin'); ?></h4>
