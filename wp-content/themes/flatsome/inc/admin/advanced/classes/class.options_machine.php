...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'presets':
					$output .= '<a href="#" class="button-primary">选择预置</a>';
				...
				case 'heading':
					...
					$menu .= '<li class="'. $header_class .'"><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'"'. $icon .'>'.  $value['desc'] .'</a></li>';
					$output .= '<div class="group" id="'. $jquery_click_hook  .'"><h2>'.$value['desc'].'</h2>'."\n";
				break;
				...
				case 'backup':
					...
					$output .= '<a href="#" id="of_backup_button" class="button" title="备份选项<">备份选项</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="恢复选项">恢复选项</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="导入选项">导入选项</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">上传</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">移除</span>';
		}
		else 
		{
			$output .= '<p class="upload-notice"><i>更新您的wordpress版本完全支持媒体功能.</i></p>';
		}
