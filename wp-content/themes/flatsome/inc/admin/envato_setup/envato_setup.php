...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_menus() {
			add_submenu_page('flatsome-panel', __( 'Setup Wizard','flatsome-admin' ), __( 'Setup Wizard','flatsome-admin' ), 'manage_options', $this->page_slug,  array( $this, $this->page_slug) );
		}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => __( 'Introduction', 'flatsome-admin' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text' => __( '...verifying','flatsome-admin' ),
			) );
		...
		<head>
			...
			<title><?php _e( 'Theme &rsaquo; Setup Wizard', 'flatsome-admin' ); ?></title>
		...
		?>
		<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress Dashboard', 'flatsome-admin' ); ?></a>
		</body>
		<?php
...
//				print_r($categories);
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php _e( 'Default Plugins', 'flatsome-admin' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p class="lead"><?php _e( 'This will install the default plugins included with Flatsome. You can add and remove plugins later on from within WordPress.', 'flatsome-admin' ); ?></p>
					...
					echo '<p class="lead">'.__( 'Good news! All plugins are already installed and up to date. Please continue.','flatsome-admin' ).'</p>';
				} ?>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php _e( 'Continue', 'flatsome-admin' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'flatsome-admin' ); ?></a>
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No Slug Found','flatsome-admin' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Activating Plugin','flatsome-admin' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Updating Plugin','flatsome-admin' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Installing Plugin','flatsome-admin' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => __( 'Success','flatsome-admin' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach($available_content as $post_type => $post_data){
				if(count($post_data)){
					...
					$content[$post_type] = array(
						...
						'description' => sprintf( __( 'This will create default %s as seen in the demo.', 'flatsome-admin' ), $post_type_title ),
						'pending' => __( 'Pending.', 'flatsome-admin' ),
						'installing' => __( 'Installing.', 'flatsome-admin' ),
			...
			$content['widgets'] = array(
				'title' => __( 'Widgets', 'flatsome-admin' ),
				'description' => __( 'Insert default sidebar widgets as seen in the demo.', 'flatsome-admin' ),
				'pending' => __( 'Pending.', 'flatsome-admin' ),
				'installing' => __( 'Installing Default Widgets.', 'flatsome-admin' ),
				'success' => __( 'Success.', 'flatsome-admin' ),
			...
			$content['settings'] = array(
				'title' => __( 'Settings', 'flatsome-admin' ),
				'description' => __( 'Configure default settings.', 'flatsome-admin' ),
				'pending' => __( 'Pending.', 'flatsome-admin' ),
				'installing' => __( 'Installing Default Settings.', 'flatsome-admin' ),
				'success' => __( 'Success.', 'flatsome-admin' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php _e( 'Install Demo Content', 'flatsome-admin' ); ?></h1>
			<form method="post">
				...
				<?php }else{ ?>
					<p class="lead"><?php printf( __( 'It\'s time to insert some default content for your new Flatsome website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'flatsome-admin' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"> </td>
						<th class="item"><?php _e( 'Item', 'flatsome-admin' ); ?></th>
						<th class="description"><?php _e( 'Description', 'flatsome-admin' ); ?></th>
						<th class="status"><?php _e( 'Status', 'flatsome-admin' ); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php _e( 'Continue', 'flatsome-admin' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'flatsome-admin' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No content Found','flatsome-admin' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'error' => 1, 'message' => __( 'Error','flatsome-admin' ) ) );
			}
...
//							$file_data = wp_remote_get($post_data['guid']);
						...
						if ( $info = wp_check_filetype( $upload['file'] ) ) {
							...
							return false;
//								return new WP_Error( 'attachment_processing_error', __( 'Invalid file type',  'flatsome-admin' ) );
						}
...
//									echo "Post meta $meta_key was $meta_val \n\n";
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( !$upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && !empty($response['body']) && $response['response']['code'] == '200' ) {
					...
					@unlink( $upload['file'] );
					return new WP_Error( 'import_file_error', __( 'Remote server did not respond',  'flatsome-admin' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size',  'flatsome-admin' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', __( 'Zero size file downloaded',  'flatsome-admin' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s',  'flatsome-admin' ), size_format( $max_size ) ) );
				}
			}
		...
		public function envato_setup_logo_design() {			
			?>
			<h1><?php _e( 'Logo &amp; Design', 'flatsome-admin' ); ?></h1>
			<h3><?php _e( 'Upload Logo', 'flatsome-admin' ); ?></h3>
			<form method="post">
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php _e( 'Upload New Logo', 'flatsome-admin' ); ?></a>
						</td>
					</tr>
				</table>
				...
				<h3 style="margin-top: 30px;"><?php _e( 'Select Preset', 'flatsome-admin' ); ?></h3>
				<?php 
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'flatsome-admin' ); ?>" name="save_step" />
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'flatsome-admin' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php _e( 'Activate Theme', 'flatsome-admin' ); ?></h1>
			<p class="lead">Enter your Purchase Code for Automatic Theme Updates and access to Support.</p>
				<?php
				    ...
				    echo '<form class="wupdates_purchase_code" action="" method="post">' .
				             ...
				             '<p class="envato-setup-actions step">' .
				              '<input type="submit" class="button button-large button-next button-primary" value="激活"/>' .
				              '<a href="'.esc_url( $this->get_next_step_link() ).'" class="button button-large button-next">'.__( 'Skip this step', 'flatsome-admin' ).'</a>'.
 				             '</p>
				      </form>';
				  	} else{
				    echo '<form class="wupdates_purchase_code" action="" method="post">' .
				             ...
				             '<input type="text" id="' . sanitize_title( $slug ) . '_wup_purchase_code" name="' . sanitize_title( $slug ) . '_wup_purchase_code"
				              value="' . $purchase_code . '" placeholder="Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )" style="width:100%; padding:10px;"/><br/><br/>' .
				              '<p class="envato-setup-actions step">' .
				              '<a href="'.esc_url( $this->get_next_step_link() ).'" class="button button-primary button-large button-next">'.__( 'Continue', 'flatsome-admin' ).'</a>' .
 				             '</p>
				      </form>';
				  	}
?>
		...
		?>
			...
			<h1>设置 Flatsome子主题 (可选)</h1>
	
			<p>
				如果您要修改主题源码，最好采用 <a href="https://codex.wordpress.org/Child_Themes" target="_blank">子主题</a> 而不是直接修改 HTML/CSS/PHP 代码. 这样更新时直接替代父主题而不影响.
			</p>

			<?php if(!isset($_REQUEST['theme_name'])){ ?>
			<p class="lead">如果不理解或不确定子主题，直接略过该步骤.</p>
			<?php } ?>
			...
			<form action="<?php $_PHP_SELF ?>" method="POST">
			 ...
			 <label style="font-weight: bold;margin-bottom: 5px; display: block;">子主题标题</label>
			...
			<p class="envato-setup-actions step">
		        <button type="submit" id= type="submit"  class="button button-primary button-next button-next">
		         <?php _e( 'Create and Use Child Theme', 'flatsome-admin' ); ?>
		        </button>
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'flatsome-admin' ); ?></a>
			...
			<?php } else { ?>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Continue', 'flatsome-admin' ); ?></a>
			</p>
			<?php } ?>
			<?php
		}
		public function envato_setup_help_support() {
			?>
			<h1>帮助和支持</h1>
			<p class="lead">这个主题自购买之日起有6个月的支持 (但可以延长). 许可证仅限于一个网站. 如果使用另一个网站需要购买附加许可证.</p>

			<p class="success">项目支持 <strong>包括</strong> :</p>
			...
			<p class="error">项目支持 <strong>不</strong> 包括:</p>
			<ul>
				<li>自定义服务 (可以通过 <a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">Envato Studio</a>)</li>
				<li>安装服务 (这适合通过 <a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">Envato Studio</a>)</li>
				<li>帮助和支持非捆绑插件 (例如你自己以后安装的)</li>
			</ul>
			<p>更多详情可在ThemeForest <a href="http://themeforest.net/page/item_support_policy" target="_blank">Item Support Polity</a>找到. </p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Agree and Continue', 'flatsome-admin' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			<h1><?php _e( 'Your Website is Ready!', 'flatsome-admin' ); ?></h1>

			<p class="lead success">恭喜! 主题已激活网站准备就绪. 登录您的 WordPress 面板修改默认内容以适合您的需要.</p>
			<p>请返回d <a href="http://themeforest.net/downloads" target="_blank">进行5星评价</a> 如果您喜欢这个主题. <br/>Follow <a  href="https://twitter.com/uxthemes" target="_blank">@uxthemes</a> on Twitter查看更新. 谢谢! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php _e( 'Next Steps', 'flatsome-admin' ); ?></h2>
					<ul>
						<?php if(class_exists('woocommerce')) { ?><li class="setup-product"><a class="button  button-primary button-large woocommerce-button" href="<?php echo admin_url().'index.php?page=wc-setup';?>"><?php _e( 'Setup WooCommerce (optional)', 'flatsome-admin' ); ?></a></li><?php } ?>
						<li class="setup-product"><a class="button button-primary button-large" href="https://www.facebook.com/groups/flatsome/" target="_blank"><?php _e( 'Join Facebook Group', 'flatsome-admin' ); ?></a></li>
						<li class="setup-product"><a class="button button-large" href="<?php echo esc_url( home_url() ); ?>"><?php _e( 'View your new website!', 'flatsome-admin' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php _e( 'More Resources', 'flatsome-admin' ); ?></h2>
					<ul>
						<li class="documentation"><a href="http://uxthemes.helpscoutdocs.com"><?php _e( 'Theme Documentation', 'flatsome-admin' ); ?></a></li>
						<li class="woocommerce documentation"><a href="https://docs.woocommerce.com/document/woocommerce-101-video-series/"><?php _e( 'Learn how to use WooCommerce', 'flatsome-admin' ); ?></a></li>
						<li class="howto"><a href="https://wordpress.org/support/"><?php _e( 'Learn how to use WordPress', 'flatsome-admin' ); ?></a></li>
						<li class="rating"><a href="http://themeforest.net/downloads"><?php _e( 'Leave an Item Rating', 'flatsome-admin' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
