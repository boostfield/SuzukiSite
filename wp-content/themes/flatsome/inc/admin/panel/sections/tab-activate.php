...
<div id="tab-activate" class="col cols panel flatsome-panel">
    ...
    if ( ! empty( $purchase_code ) ) {
      if ( ! empty( $errors ) ) {
        ...
        $purchase_code = false;
        echo '<div class="notice-warning notice-alt"><p>' . esc_html__( 'Purchase code removed.' ,'flatsome-admin') . '</p></div>';
      } else {
        //this means a valid purchase code is present and no errors were found
       echo '<div class="notice-success notice-alt notice-large" style="margin-bottom:15px!important">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy Flatsome Theme and automatic updates.','flatsome-admin') . '</div>';
      }
    }

    if ( empty( $purchase_code ) ) {
    echo '<form class="wupdates_purchase_code" action="" method="post">' .
             __( '<p>Enter your purchase code and <strong>hit return/enter</strong>. Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.</p>','flatsome-admin') .
