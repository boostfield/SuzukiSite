...
class Flatsome_UX_Blocks_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'block_widget', 'description' => __('A widget that displays a Block ', 'flatsome-admin'), 'customize_selective_refresh' => true);

		$control_ops = array('id_base' => 'block_widget' );

		parent::__construct( 'block_widget', __('Flatsome Blocks', 'flatsome-admin'), $widget_ops, $control_ops );
	}
...
?>
		...
		<p><label for="<?php echo $this->get_field_id( 'block' ); ?>"><?php _e( 'Block:', 'flatsome-admin' ); ?></label>
		...
		<?php foreach ($blocks as $key => $value) {
 		...
 		<p>如果您在前端停在上面，您可以使用UX构建器编辑这个块。<a href="#">了解更多关于这块。</a></p>
<?php
	}
}
