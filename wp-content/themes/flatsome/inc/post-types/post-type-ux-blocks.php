...
register_post_type( 'blocks',
	array(
		'labels'              => array(
			'add_new_item'       => __( 'Add block', "flatsome-admin" ),
			'name'               => __( 'Blocks', "flatsome-admin" ),
			'singular_name'      => __( 'Block', "flatsome-admin" ),
			'edit_item'          => __( 'Edit Block', "flatsome-admin" ),
			'view_item'          => __( 'View Block', "flatsome-admin" ),
			'search_items'       => __( 'Search Blocks', "flatsome-admin" ),
			'not_found'          => __( 'No Blocks found', "flatsome-admin" ),
			'not_found_in_trash' => __( 'No Blocks found in Trash', "flatsome-admin" ),
		),
		'public'              => true,
		'has_archive'         => false,
		'show_in_menu'        => true,
...
function my_edit_blocks_columns() {
	$columns = array(
		...
		'shortcode' => __( 'Shortcode', 'flatsome-admin' ),
		'date'      => __( 'Date', 'flatsome-admin' ),
	);
...
function block_shortcode( $atts, $content = null ) {
	...
	if ( $post_id ) {
		...
		if ( empty( $html ) ) {
            $html = '<p class="lead shortcode-error">在UX编辑器中打开添加并修改此内容</p>';
		}
		...
		if ( isset( $post ) && current_user_can( 'edit_pages' )
			...
			$html              = '<div class="block-edit-link" data-title="编辑块: ' . get_the_title( $post_id ) . '"   data-backend="' . esc_url( $edit_link_backend )
