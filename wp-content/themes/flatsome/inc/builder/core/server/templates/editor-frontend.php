...
if ( is_network_admin() ) {
  ...
  $admin_title = sprintf( __( 'Network Admin: %s' , 'flatsome-admin'), esc_html( get_network()->site_name ) );
} elseif ( is_user_admin() ) {
  /* translators: User dashboard screen title. 1: Network name */
  $admin_title = sprintf( __( 'User Dashboard: %s' , 'flatsome-admin'), esc_html( get_network()->site_name ) );
} else {
...
if ( $admin_title == $title ) {
  ...
  $admin_title = sprintf( __( '%1$s &#8212; WordPress' , 'flatsome-admin'), $title );
} else {
  /* translators: Admin screen title. 1: Admin screen name, 2: Network or site name */
  $admin_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; WordPress' , 'flatsome-admin'), $title, $admin_title );
}
