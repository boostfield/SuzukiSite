...
function flatsome_shop_widgets_init() {
  ...
  register_sidebar( array(
    'name'          => __( 'Shop Sidebar', 'flatsome-admin' ),
    'id'            => 'shop-sidebar',
  ...
  register_sidebar( array(
    'name'          => __( 'Product Sidebar', 'flatsome-admin' ),
    'id'            => 'product-sidebar',
    'before_widget' => '<aside id="%1$s" class="widget %2$s">',
    'after_widget'  => '</aside>',
    'before_title'  => '<span class="widget-title shop-sidebar">',
...
function flatsome_woocommerce_breadcrumbs() {
    ...
    $home = (get_theme_mod('breadcrumb_home',1)) ? _x( 'Home', 'breadcrumb', 'flatsome-admin' ) : false;
...
if ( ! function_exists( 'flatsome_header_add_to_cart_custom_icon_fragment_count_label' ) ) {
	...
	function flatsome_header_add_to_cart_custom_icon_fragment_count_label( $fragments ) {
		...
		<span class="image-icon header-cart-icon" data-icon-label="<?php echo $woocommerce->cart->cart_contents_count; ?>">
			<img class="cart-img-icon" alt="<?php _e( 'Cart', 'flatsome-admin' ); ?>" src="<?php echo do_shortcode( $custom_cart_icon ); ?>"/>
		</span>
		<?php
...
function flatsome_pages_in_search_results(){
    ...
    <?php
      ...
      if(!empty($posts) || !empty($pages)){
        echo '<hr/>';
        echo '<h4 class="uppercase">'.__('Pages and posts found','flatsome-admin').'</h4>';
...
function flatsome_presentage_bubble($product){
  ...
  if($product->is_type( 'simple' ) ||$product->is_type( 'external' )){
      $price = round( ( ( floatval($regular_price) - floatval($sale_price) ) / floatval($regular_price) ) * 100 );
    ...
    $price = sprintf( __('%s', 'flatsome-admin' ), $maximumper);
  } else {
    $price = __( 'Sale!', 'flatsome-admin' );
  }
  if (get_theme_mod('sale_bubble_percentage_formatting')) {
  $formatting = get_theme_mod('sale_bubble_percentage_formatting' , '-{value}%' );
    } else {
  $formatting = '-{value}%';      
    }
  $price =  str_replace( '{value}', $price, $formatting);
  return $price;
}
