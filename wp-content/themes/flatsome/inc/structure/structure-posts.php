...
function flatsome_content_nav( $nav_id ) {
   ...
   <?php if ( get_next_posts_link() ) : ?>
        <div class="nav-previous"><?php next_posts_link( __( '<span class="icon-angle-left"></span> Older posts', 'flatsome-admin' ) ); ?></div>
        <?php endif; ?>
		</div>
		<div class="flex-col flex-grow">
		  <?php if ( get_previous_posts_link() ) : ?>
		     <div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="icon-angle-right"></span>', 'flatsome-admin' ) ); ?></div>
		 <?php endif; ?>		</div>
	</div>
	<?php endif; ?>
...
function flatsome_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'flatsome-admin' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'flatsome-admin' ), '<span class="edit-link">', '<span>' ); ?></p>
	<?php
			break;
		default :
	?>
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment-inner">
            ...
            <div class="flex-row align-top">
                ...
                <div class="flex-col flex-grow">
                    <?php printf( __( '%s <span class="says">says:</span>', 'flatsome-admin' ), sprintf( '<cite class="strong fn">%s</cite>', get_comment_author_link() ) ); ?>
                    <?php if ( $comment->comment_approved == '0' ) : ?>
                    <em><?php _e( 'Your comment is awaiting moderation.', 'flatsome-admin' ); ?></em>
                     <br />
                    <?php endif; ?>
                 ...
                 <div class="comment-meta commentmetadata uppercase is-xsmall clear">
                    ...
                    <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'flatsome-admin' ), get_comment_date(), get_comment_time() ); ?>
                    </time></a>
                    <?php edit_comment_link( __( 'Edit', 'flatsome-admin' ), '<span class="edit-link ml-half strong">', '<span>' ); ?>
...
function flatsome_posted_on() {
    ...
    $posted_on = sprintf(
        esc_html_x( 'Posted on %s', 'post date', 'flatsome-admin' ),
        '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
    );

    $byline = sprintf(
        esc_html_x( 'by %s', 'post author', 'flatsome-admin' ),
